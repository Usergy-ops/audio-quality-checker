...
function downloadPdf() {
    ...
    const compHtml = (comp.checks || []).map(c => {
        ...
        return `<tr><td>${icon} ${c.metric}</td><td>${c.value}</td><td>${c.threshold}</td><td>${c.message}</td></tr>`;
    }).join('');

    const vizHtml = ['waveform','spectrogram','loudness','speakers'].map(k =>
        viz[k] ? `<div style="margin:12px 0"><img src="data:image/png;base64,${viz[k]}" style="width:100%;border-radius:8px"></div>` : ''
    ).join('');

    const html = `<!DOCTYPE html><html><head><meta charset="UTF-8">
<title>Audio Quality Report - ${fi.filename || 'Unknown'}</title>
<style>
  body{font-family:-apple-system,system-ui,sans-serif;max-width:900px;margin:0 auto;padding:20px;color:#1c1c1c;font-size:14px}
  h1{color:#00897B;border-bottom:2px solid #00897B;padding-bottom:8px}
  h2{color:#333;margin-top:24px;font-size:1.2rem}
  .section{margin-bottom:20px;page-break-inside:avoid}
  table{width:100%;border-collapse:collapse;margin:8px 0}
  th,td{padding:6px 10px;text-align:left;border-bottom:1px solid #e8e8e8;font-size:0.9rem}
  th{background:#f5f5f5;font-weight:600}
  .score-big{font-size:3rem;font-weight:700;display:inline-block}
  .meta{color:#888;font-size:0.85rem}
  @media print{body{padding:0} .no-print{display:none}}
</style></head><body>
<h1>Audio Quality Report</h1>
<p class="meta">Generated by <strong>UsergyAI Audio Quality Checker</strong> on ${new Date().toLocaleDateString()} at ${new Date().toLocaleTimeString()}</p>
...
<div class="section">
  ...
  <div style="display:flex;align-items:center;gap:16px">
    <div class="score-big" style="color:${scoreColor}">${q.score || 0}</div>
    <div><strong style="font-size:1.5rem">${q.grade || '-'}</strong><br>${q.summary || ''}</div>
  </div>
</div>
...
<hr style="margin-top:30px;border:none;border-top:1px solid #e8e8e8">
